...
class MyFilter implements Filter<Tree> {
	private static final long serialVersionUID = 8063741548313970150L;

	@Override
	public boolean accept(Tree t) {
		if (t.nodeString().startsWith("SBAR"))
			return false;
		return true;
	}
...
class Tuple implements Comparable<Tuple> {
	...
	public int compareTo(Tuple t) {
		if (score > t.score)
			return 1;
		else if (Math.abs(score - t.score) < EPS)
			return 0;
		return -1;
	}

	boolean equals(Tuple t) {
		if (score == t.score && t.word.compareTo(word) == 0)
			return true;
		return false;
	}
...
public class SummaryGenerator {
			...
			if (firstSentence) {
				...
				for (CoreMap sentence : sentences) {
					...
					for (Map.Entry<String, Double> entry : tf.entrySet()) {
						...
						double cscore = Math.log(tfreq + 1)
	...
	private String compressSentence(String sentence) {
		...
		/*
		 * listDep.addAll(dependencies.findAllRelns(EnglishGrammaticalRelations.
		 * ADJECTIVAL_MODIFIER));
		 * listDep.addAll(dependencies.findAllRelns(EnglishGrammaticalRelations
		 * .AGENT));
		 * listDep.addAll(dependencies.findAllRelns(EnglishGrammaticalRelations
		...
		/*
		 * Set<IndexedWord> leaves = dependencies.getLeafVertices(); for
		 * (IndexedWord leaf : leaves) dependencies.removeVertex(leaf);
		 * 
		 * sentence = dependencies.toRecoveredSentenceString();
		 */
